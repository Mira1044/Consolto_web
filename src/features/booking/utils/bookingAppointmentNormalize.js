/**
 * Pure helpers for normalizing API appointment rows into UI booking cards.
 */
...
/**
 * @param {Array} rawAppointments - raw appointment rows from API
 * @returns {{ upcoming: Array, past: Array }}
 */
export function splitAppointmentsIntoUpcomingPast(rawAppointments) {
  const normalized = (rawAppointments || []).map((a, i) => normalizeAppointment(a, i));
  return {
    upcoming: normalized.filter((b) => b.status === 'upcoming'),
    past: normalized.filter((b) => b.status !== 'upcoming'),
  };
}
