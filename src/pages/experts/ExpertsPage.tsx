...
function VerifiedBadge({ size = 16 }: { size?: number }): JSX.Element {
  return (
    <svg
...
export const ExpertsPage: React.FC = () => {
  ...
  // useAuth throws if context is null, so it never returns null
  ...
  const handleBook = (expert: ExpertType, duration: number): void => {
    void navigate('/booking', { state: { expert, duration } });
  };

  const visibleCats = showAll ? categories : categories.slice(0, 6);

  const filteredExperts = experts.filter((e: ExpertType): boolean => {
    const matchSearch =
      !search ||
